...
import { ArrowLeft, BrainCircuit, ChevronRight, Cpu, MemoryStick, Monitor, Zap, type LucideIcon } from "lucide-react";
...
export default function Optimizer({ onNavigate: _onNavigate }: { onNavigate: (page: PageId) => void }) {
  ...
  // ---- Landing: hardware components first, then categories ----
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* By category */}
      <div>
        <h2 className="mb-3 text-[13px] font-bold uppercase tracking-wide text-txt2">By category</h2>
        <div className="grid grid-cols-3 gap-4">
          {CATEGORY_ORDER.map((c, i) => {
            ...
            return (
              <button
                key={c}
                onClick={() => { setSelected(new Set()); setOpenCat(c); }}
                style={{ animationDelay: `${50 + i * 40}ms` }}
                className="stagger-item relative flex flex-col items-start rounded-2xl border border-edge bg-card p-6 text-left transition-all hover:-translate-y-0.5 hover:scale-[1.02] hover:border-accent/30 hover:shadow-[0_0_20px_rgba(227,0,14,0.08)]"
              >
